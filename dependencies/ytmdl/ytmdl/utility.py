...
def exe(command):
    ...
    c = command.split()
    output, error = subprocess.Popen(c, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
...
def convert_to_mp3r(path):
    ...
    try:
        ...
        command = "ffmpeg -loglevel panic -i {} -vn -ar 44100 -ac 2 -ab {}k -f mp3 {}".format(
            path, defaults.DEFAULT.SONG_QUALITY, new_name
        )
...
def convert_to_mp3(path, start=None, end=None, cleanup_after_done=True):
    ...
    new_name = path + "_new.mp3"
    params = {"loglevel": "panic", "ar": 44100, "ac": 2, "ab": "{}k".format(defaults.DEFAULT.SONG_QUALITY), "f": "mp3"}
...
def extract_m4a(path, start=None, end=None, cleanup_after_done=True):
    ...
    try:
        job = ffmpeg.input(path).output(new_name, ss=start, to=end, loglevel="panic")
        job.aio()
...
def extract_part_convert(path, format, start, end):
    ...
    """
    FORMAT_MAP = {"mp3": convert_to_mp3, "opus": convert_to_opus, "m4a": extract_m4a}
...
def get_new_title(old_title):
    ...
    """
    logger.info("Current extracted title for the song is: `{}`".format(old_title))
    logger.info("Most extracted titles are not accurate and they affect the meta search")
...
def get_new_meta_search_by(old_search_by):
    ...
    """
    prepend.PREPEND(1)
    new_search_by = Prompt.ask("Enter new title to search metadata with", default=old_search_by)

    if new_search_by == old_search_by:
        logger.info("You can disable this behaviour by removing the" "`--ask-meta-name` flag.")
