...
def mark_beat(output_spectrogram_path, input_audio_path, input_spectrogram_path, mark_radius=0):
    ...
    try:
        ...
        for beat_col in beat_cols:
            ...
            img[:, beat_col - mark_radius : beat_col + mark_radius + 1, 0] = 255
...
if __name__ == "__main__":
    ...
    os.makedirs(DATASET_DIR / "spectrogram" / "vocals_beat_mark_width_5", exist_ok=True)
    for i, row in split_by_time_sample.iterrows():
        mark_beat(
            DATASET_DIR / "spectrogram" / "vocals_beat_mark_width_5" / f"{row['music_name']}.png",
